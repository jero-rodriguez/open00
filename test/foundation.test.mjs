...
import { evaluateRelease, validateBuildInput, validateCommitState, validateConventionalCommit, validateRepositorySelector, auditPackageContents } from '../tools/release-guard.mjs';
...
test('builds an installable manifest with exact Foundry 14.367 compatibility', () => {
  ...
  buildPackage(root);
  const manifest = JSON.parse(readFileSync(join(root, 'dist/system.json'), 'utf8'));
  ...
  assert.deepEqual(
    ...
    },
    {
      id: 'open00',
      title: 'open00',
      description: 'Private-use open00 system.',
      packageName: 'open00-foundry-v14',
    },
  );
...
test('release guard rejects unsafe package inputs and any non-private release state', () => {
  for (const path of ['requirements.txt', 'CMakeLists.txt', 'guide.mdx', 'README.sh']) {
    assert.equal(validateBuildInput(path), false, path);
  }
  assert.equal(validateBuildInput('src/domain/roll-resolution.mjs'), true);
  for (const state of [
    {},
...
import { runtimeSmoke } from '../tools/runtime-smoke.mjs';
